...
class SolarmanSofarInverter(BaseInverter):
    ...
    def get_energy_data(self):
        ...
        power = [
            self.get_state_float("activepower_pcc_r") * -1,
            self.get_state_float("activepower_pcc_s") * -1,
            self.get_state_float("activepower_pcc_t") * -1,
        ]
        today = self.get_state_float("today_energy_import")
        total = 0.0  # As per payload
        current = [
            self.get_state_float("current_pcc_r"),
            self.get_state_float("current_pcc_s"),
            self.get_state_float("current_pcc_t"),
        ]

        # Define voltage as self, because we need it in another function to calculate current from power
        self.voltage = [
            self.get_state_float("voltage_phase_r"),
            self.get_state_float("voltage_phase_s"),
            self.get_state_float("voltage_phase_t"),
        ]
        frequency = self.get_state_float("grid_frequency_2")
    ...
    def get_metrics_data(self):
        ...
        combined_power = round(self.get_state_float("activepower_load_sys") / 3)
        load_power = [combined_power] * 3

        alarm_codes = [0]
        battery_soc = self.get_state_int("battery_soc_average")
        ...
        battery_power = [self.get_state_float("battery_power_total")]
        ...
        grid_export_limit = self.get_state_float("export_surplus_power")
        battery_temperature = [self.get_state_float("battery_temperature")]
        inverter_temperature = self.get_state_float("ambient_temperature_1")
